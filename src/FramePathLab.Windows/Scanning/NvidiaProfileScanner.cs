...
///
/// The driver's own library is loaded from the protected Windows system directory, so no binary
/// is bundled and the current directory cannot substitute a DLL. Every call is read-only: the settings framework is opened, the profile is
...
public static class NvidiaProfileScanner
{
    ...
    {
        var driverLibrary = Path.Combine(Environment.SystemDirectory, "nvapi64.dll");
        if (!File.Exists(driverLibrary) || !NativeLibrary.TryLoad(driverLibrary, out var library))
        {
