...
public static class ExpertTweakCatalog
{
    ...
    private static ExpertTweakCard CorePlacement(ExpertScanContext context)
    {
        ...
        // Deliberately not applied by writing into the running game.
        //
        // Setting affinity on a live game process means opening a handle to it with rights that
        // let the caller change its execution. Nothing about that is cheating, but an anti-cheat
        // that watches for processes reaching into the game cannot tell intent from behaviour, and
        // for a player whose account is their livelihood that asymmetry is not worth a few percent.
        // The launch-time route reaches the same placement without ever touching the process.
        var launchCommand = $"cmd /c start \"\" /affinity {preferredMask:X} \"<path to {context.GameExecutableName}.exe>\"";
        var current = cpu.GameAffinityMask;
        var alreadyPlaced = current == preferredMask;

        return new ExpertTweakCard(
            definition,
            new TweakReading(
                alreadyPlaced ? TweakState.Optimal : TweakState.Blocked,
                current is null
                    ? "Game is not running; placement not observed"
                    : $"0x{current:X} ({System.Numerics.BitOperations.PopCount(current.Value)} logical processors)",
                recommended,
                alreadyPlaced
                    ? "The game is already confined to the preferred core group."
                    : cpu.PreferredGroupReason
                      + " Apply this at launch rather than to a running game: "
                      + launchCommand
                      + " — or set the same mask in a launcher. FramePath Lab will not write affinity into a "
                      + "running game process, because reaching into a protected process is a risk that is not "
                      + "worth taking for a placement that can be set at launch instead."),
            [],
            alreadyPlaced
                ? null
                : "Applied at launch by the user; this application does not modify the running game process.");
    }
    ...
    private static ExpertTweakCard AudioSampleRate(ExpertScanContext context)
    {
        var definition = new ExpertTweakDefinition(
            ...
            "Engines author and mix at 48 kHz. Resampling adds a processing stage and softens the transient edge "
            + "of a sound, and the leading edge is exactly what a footstep's direction is judged from.",
        ...
        return new ExpertTweakCard(
            definition,
            new TweakReading(
                endpoint.IsResampling ? TweakState.Suboptimal : TweakState.Optimal,
                $"{endpoint.FriendlyName}: {endpoint.SampleRateHz / 1000d:0.###} kHz, {endpoint.BitsPerSample}-bit, "
                + $"{endpoint.Channels} channel(s)",
                "48 kHz, 24-bit",
                endpoint.IsResampling
                    ? $"The endpoint mixes at {endpoint.SampleRateHz / 1000d:0.###} kHz, so every buffer is "
                      + "resampled. Set the device to 24-bit 48000 Hz in the sound control panel."
                    : "The shared format already matches the rate games render at."),
            [],
            endpoint.IsResampling ? "The endpoint format is set in the Windows sound control panel." : null);
    }
    ...
    private static ExpertTweakCard AudioEffects(ExpertScanContext context)
    {
        var definition = new ExpertTweakDefinition(
            ...
            "Dynamic-range processing is directly hostile to competitive listening. Loudness equalisation raises "
            + "quiet sounds and lowers loud ones, which destroys the amplitude difference that distance is "
            + "judged from, and every stage adds pipeline delay.",
        ...
        return new ExpertTweakCard(
            ...
            new TweakReading(
                ...
                disabled
                    ...
                    : "Endpoint effects are active. Disable enhancements for this device and re-scan."),
            [],
            disabled ? null : "Endpoint effects are switched off in the Windows sound control panel.");
    }
    ...
    private static ExpertTweakCard NetworkPathStability(ExpertScanContext context)
    {
        var definition = new ExpertTweakDefinition(
            ...
            + "packet crosses.",
            "Bandwidth almost never limits a competitive session; jitter does. A link that averages an acceptable "
            + "round-trip but scatters it moves the arrival time of every server tick, and that is experienced as "
            + "shots failing to register rather than as a network fault. The gateway hop is where a wireless "
            + "link, a failing cable or a saturated uplink actually shows.",
    ...
    private static ExpertTweakCard FrameLimiterStrategy(ExpertScanContext context)
    {
        var definition = new ExpertTweakDefinition(
            ...
            "The limiter closest to the engine holds the least queued work. An engine-level or vendor "
            + "latency-path limiter typically runs around half a frame to a full frame lower latency than an "
            + "external one, while an external limiter targeting a fixed frame time can pace more evenly. Which "
            + "matters more depends on whether the complaint is latency or cadence.",
    ...
    private static ExpertTweakCard FastStartup(ExpertScanContext context, ITweakStateReader reader)
    {
        var definition = new ExpertTweakDefinition(
            ...
            "This is why a problem survives a shutdown but vanishes after a restart. For a machine that is tuned "
            + "and then expected to behave identically every session, resuming a stale kernel session is the "
            + "opposite of what is wanted.",
            "Cold boots take slightly longer. No other behaviour changes.",
            TweakRisk.Low,
            TweakScope.Machine,
            EvidenceQuality.Moderate,
            true,
        ...
        return new ExpertTweakCard(
            ...
            new TweakReading(
                ...
                context.DefenderExclusions.Count > 0
                    ? string.Join("; ", context.DefenderExclusions.Take(5))
    ...
    private static IEnumerable<ExpertTweakCard> NvidiaProfileCards(ExpertScanContext context)
    {
        ...
        var offTarget = profile.Settings.Where(setting => !setting.IsOptimal).ToArray();
        yield return new ExpertTweakCard(
            definition,
            new TweakReading(
                offTarget.Length == 0 ? TweakState.Optimal : TweakState.Blocked,
                string.Join("; ", profile.Settings.Select(setting => $"{setting.Name}: {setting.Value}")),
                string.Join("; ", profile.Settings.Select(setting => $"{setting.Name}: {setting.Recommended}")),
                profile.Observation + (offTarget.Length == 0
                    ? " Every readable setting is already at the value this catalogue would choose."
                    : $" {offTarget.Length} setting(s) differ from the competitive default: "
                      + string.Join(", ", offTarget.Select(setting => setting.Name))
                      + ". Change them in the driver control panel, then re-scan.")),
            [],
            offTarget.Length == 0 ? null : "Driver profile settings are changed in the vendor control panel.");
    }
